...
def demo():
    ...
    post("/api/products/", {"name": "crispy chips", "price": 4.99})
    post("/api/products/", {"name": "sweet candies", "price": 3.49})
    ...
    put(f"/api/orders/{order_id_ok}", {"process": True})  # Process OK order
    print("OK order processed successfully")
    put("/api/orders/2", {"process": True})  # NOK order (reject)

    order_id_ok = 3  # Replace with the order ID
    update_customer_balance(order_id_ok)
    put("/api/orders/3", {"process": True})  # NOK order (default)
